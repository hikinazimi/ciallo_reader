...
class PageFlipWidget extends StatefulWidget {
  ...
  // 🔥 新增：定义回调函数类型 (接收一个 int 页码)
  final ValueChanged<int>? onPageFlip;
...
class PageFlipWidgetState extends State<PageFlipWidget> with TickerProviderStateMixin {
  ...
  // 这些变量在你提供的代码中似乎是全局变量或混入变量
  // 我保留原样，防止报错，如果报错请检查你的 import
  // (假设 imageData, currentAmount 等是在外部定义的)
  ...
  Future nextPage() async {
    ...
    if (mounted) {
      setState(() {
        pageNumber++;
        // 🔥 新增：触发回调通知父组件
        widget.onPageFlip?.call(pageNumber);
      });
    }
